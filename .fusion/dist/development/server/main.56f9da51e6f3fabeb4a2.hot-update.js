...
const createAlbum = (name, images) => {
  ...
  console.log("images.imagePreviewUrl", images.imagePreviewUrl);
  const imagesArray = [];
  const albumData = {
    file: images.file.name,
    imagePreviewUrl: images.imagePreviewUrl
  };
  imagesArray.push(albumData);
  console.log("imagesArray", imagesArray);
  return dispatch => {
    axios__WEBPACK_IMPORTED_MODULE_0___default.a.post(`${ROOT_URL}/create-album`, imagesArray).then(res => {
      console.log("res", res);
      dispatch({
        type: CREATE_ALBUM,
        payload: res.data
...
//# sourceMappingURL=main.56f9da51e6f3fabeb4a2.hot-update.js.map